-- Reko: SECURITY DEFINER functions for sale/stock/payment side-effects.
-- These bypass RLS by design, so every function MUST explicitly verify
-- the caller's shop_id before touching any row. This check replaces RLS
-- inside these functions — it is not optional.
...
create or replace function restock_product(p_product_id uuid, p_quantity integer, p_reason text default 'restock')
returns products
language plpgsql
security definer
set search_path = public
...
create or replace function record_payment(p_sale_id uuid, p_amount numeric default null, p_pay_full boolean default false)
returns sales
language plpgsql
security definer
set search_path = public
...
-- Explicit, narrow grants: only authenticated shop members can call these.
-- No grants to anon.
revoke all on function create_sale(uuid, text, text, integer, numeric, numeric, text) from public;
revoke all on function update_sale(uuid, integer, numeric, numeric, text) from public;
revoke all on function delete_sale(uuid) from public;
revoke all on function restock_product(uuid, integer, text) from public;
revoke all on function record_payment(uuid, numeric, boolean) from public;

grant execute on function create_sale(uuid, text, text, integer, numeric, numeric, text) to authenticated;
grant execute on function update_sale(uuid, integer, numeric, numeric, text) to authenticated;
...
grant execute on function record_payment(uuid, numeric, boolean) to authenticated;
